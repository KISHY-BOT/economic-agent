...
logger.info(f"SSL context loaded with {len(ssl_context.get_ca_certs())} certificates")
...
if attempt == 2:
raise
logger.warning(f"SSL/Connection error (attempt {attempt+1}): {str(e)}")
...
codigo_entidad: int = Path(..., title="Código de entidad bancaria"),
numero_cheque: int = Path(..., title="Número de cheque"),
x_api_key: Optional[str] = Header(default=None),
request: Request = None
):
_require_api_key(x_api_key); _rate_limit(request.client.host if request and request.client else "anon")
...
def deudores(
identificacion: str = Path(..., regex=r"^\d{11}$", title="Identificación (11 dígitos sin guiones)"),
x_api_key: Optional[str] = Header(default=None),
request: Request = None
):
_require_api_key(x_api_key); _rate_limit(request.client.host if request and request.client else "anon")
...
moneda: str = Path(..., title="Código de moneda ISO (ej: USD, EUR)"),
fechadesde: Optional[str] = Query(None, description="Fecha inicial en formato YYYY-MM-DD"),
fechahasta: Optional[str] = Query(None, description="Fecha final en formato YYYY-MM-DD"),
x_api_key: Optional[str] = Header(default=None),
request: Request = None
):
_require_api_key(x_api_key); _rate_limit(request.client.host if request and request.client else "anon")
...
def monetarias(
id_var: int = Path(..., title="ID de variable monetaria"),
desde: Optional[str] = Query(None, description="Fecha inicial en formato YYYY-MM-DD"),
hasta: Optional[str] = Query(None, description="Fecha final en formato YYYY-MM-DD"),
x_api_key: Optional[str] = Header(default=None),
request: Request = None
):
_require_api_key(x_api_key); _rate_limit(request.client.host if request and request.client else "anon")
...
variable_id: int = Path(..., title="ID de variable"),
x_api_key: Optional[str] = Header(default=None),
request: Request = None
):
_require_api_key(x_api_key); _rate_limit(request.client.host if request and request.client else "anon")
...
path: str = Query(..., description="Ruta BCRA comenzando con /"),
x_api_key: Optional[str] = Header(default=None),
request: Request = None
):
_require_api_key(x_api_key); _rate_limit(request.client.host if request and request.client else "anon")
